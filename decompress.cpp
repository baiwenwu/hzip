...
inline u32 isLeafNode(waveletTree root)
{
	if (root->leftChild == NULL&&root->righChild == NULL)
	{
		return root->label;
...
int recurWaveletTreeForL(waveletTree root, uchar**buff)
{
	u32 li = 0;
	uchar *Lbuff = NULL;
	u32 ri = 0;
	uchar *Rbuff = NULL;
	//左边
	if (isLeafNode(root->leftChild)==1000)
	{
		recurWaveletTreeForL(root->leftChild, &Lbuff);
	}
	else
	{
		uchar lchar = isLeafNode(root->leftChild);
		//Lbuff = new uchar[root->bitLen];
		Lbuff =(uchar*)malloc(root->bitLen);
		memset(Lbuff, lchar, root->bitLen);
	}
	//右边
	if (isLeafNode(root->righChild) == 1000)
	{
		recurWaveletTreeForL(root->righChild, &Rbuff);
	}
	else
	{
		uchar rchar = isLeafNode(root->righChild);
		//Rbuff = new uchar[root->bitLen];
		Rbuff=(uchar*)malloc(root->bitLen);
		memset(Rbuff, rchar, root->bitLen);
	}
	uchar *tmpL =(uchar*)malloc(root->bitLen);
	uchar oneBits = 128;
	for(u32 i = 0; i < root->bitLen; i++)
	{
		if (root->ptr[i >> 3] & (128 >> (i & 7)))
			tmpL[i] = Rbuff[ri++];
		else
			tmpL[i] = Lbuff[li++];
	}
...
int genBwtWithWaveletTree_t(waveletTree root, Stream_t *streamPtr)
{
	if (!root || !streamPtr)
	{
		return ERR_PARAMETER;
	...
	uchar *bwtL = NULL;
	recurWaveletTreeForL(root, &bwtL);
	for (u32 i = 0; i < root->bitLen; i++)
		streamPtr->bwt[i] = bwtL[i];
	//streamPtr->myFree(bwtL);
	free(bwtL);
	bwtL=NULL;
	return  0;
}
