...
void *childThread(void *arg){
    ...
    while ( blkCount<threadInfos[index].nblocks)
    {
        ...
        u32 bwtIdnex_t=stream.bwtIndex;
     ...
     if(stream.treeShape==HUFFMAN)
       {
            stream.bwtIndex=bwtIdnex_t;
            for(int k=0;k<256;k++)
            {
                stream.charMap[k]=false;
                 int m=strlen((const char*)stream.codeTable[k]);
                 if(m)
                 {
                    stream.charMap[k]=true;
                 }
            }
...
void decompressChildProcess(int i,Stream_t *streamPtr)
{
    ...
    while(count<threadInfos[i].nblocks)
    {
        ...
        {
            uchar nbits_t=(streamPtr->bwtIndex>>28)&0xf;
            HBblockSize=1;
            HBblockSize=HBblockSize<<nbits_t;
            streamPtr->bwtIndex=streamPtr->bwtIndex&0xfffffff;
            //printf("\n\t???????????????????? BwtIndex=%ld\n",streamPtr->bwtIndex);
